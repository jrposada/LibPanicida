...
function Debug.DisableQuestLogging()
  ...
  EVENT_MANAGER:UnregisterForEvent(
    "LibPanicida_QuestRemoved_Debug",
    EVENT_QUEST_REMOVED
  )
...
--- Enables set ID display in Item Sets Book headers.
--- @return boolean True if enabled, false if already enabled
function Debug.EnableSetIdLogging()
  ...
  Debug.LogLater("Set ID logging enabled")

  ZO_PostHook(ZO_ItemSetsBook_Shared, "SetupGridHeaderEntry",
    function(self, control, data, selected)
      if not debugStates.setIdLogging then return end

      local itemSetHeaderData = data.header
      if not itemSetHeaderData then return end

      local setId = itemSetHeaderData:GetId()
      if not setId then return end

      local setName = itemSetHeaderData:GetFormattedName()
      control.nameLabel:SetText(string_format("%s (ID: %d)", setName, setId))
    end)
...
--- Disables set ID display in Item Sets Book headers (requires UI reload).
